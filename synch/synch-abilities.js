...
module.exports.synchabilities = (event, context, callback) => {
  ...
  function cb(kms_obj) {
    ...
    db.query(`select soc_code, attr_id, attr_name, attr_level, attr_importance from ${config_db.schema}.occ_ability;`, (err, res) => {
      ...
      for (let i = 0; i < rowCount; i++) {
        ...
        if ((previousSoc_Code.length > 0 && previousSoc_Code !== element.soc_code) || (i > 0 && i + 1 === rowCount)) {
          ...
          pipeline.hset(`details:${element.soc_code}`,
            'abilities', JSON.stringify(aAbilities));

          console.log(aAbilities);
